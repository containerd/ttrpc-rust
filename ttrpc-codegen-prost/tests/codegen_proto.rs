...
fn generate_from_shared_protos() {
    ...
    let out = std::env::temp_dir().join("ttrpc-codegen-prost-test/grpc-out");
...
fn generate_async_from_shared_protos() {
    ...
    let out = std::env::temp_dir().join("ttrpc-codegen-prost-test/grpc-async-out");
    std::fs::create_dir_all(&out).unwrap();
    Codegen::new()
        .out_dir(&out)
        .inputs(&protos)
...
fn generate_async_no_streaming() {
    ...
    let out = std::env::temp_dir().join("ttrpc-codegen-prost-test/grpc-async-nostream");
    std::fs::create_dir_all(&out).unwrap();
    Codegen::new()
        .out_dir(&out)
        .inputs(&protos)
